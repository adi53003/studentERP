...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/erp'
db=SQLAlchemy(app)
...
class stud_view(db.Model):
    __tablename__ = 'stud_view'
    id=db.Column(db.Integer,primary_key=True)
    rollno=db.Column(db.String(50))
    sname=db.Column(db.String(50))
...
@app.route('/usnlist')
def usnlist():
    #query=db.engine.execute(f"SELECT * FROM `student_view`") 
    #query=stud_view.query.all() 
    query=stud_view.query.all()
    return render_template('usnlist.html',query=query)
